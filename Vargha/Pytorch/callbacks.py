...
def get_callbacks(model_dir='logs/checkpoint', model_name='resnet-101.pth', reduce_lr_p=10, early_stopping_p=25,
                  log_dir='logs/scalars',
                  csv_dir='logs/csv'):
    # TensorBoard

    model_checkpoint = ModelCheckpoint(model_dir, filename=model_name, monitor='val_loss', save_best_only=True,
                                       verbose=1)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=reduce_lr_p, verbose=1)

    early_stopping = EarlyStopping(monitor='val_loss', patience=early_stopping_p)

    csv_logger = CSVLogger(file=csv_dir)

    return model_checkpoint, reduce_lr, early_stopping, csv_logger
